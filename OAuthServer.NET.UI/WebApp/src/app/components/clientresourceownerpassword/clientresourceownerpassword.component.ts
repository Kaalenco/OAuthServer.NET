...
@Component({
  ...
  templateUrl: './clientresourceownerpassword.component.html',
  styleUrls: ['./clientresourceownerpassword.component.scss']
})
export class ClientresourceownerpasswordComponent extends BaseIdComponent{
  grantId: string;
  clientName: string;
  clientId: string;
  clientSecret: string;
  ...
  createClient() {
    ...
    if (this.id && this.id !== '') {
      const request: IClientResourceOwnerPasswordUpdateRequest = {
      ...
      this.http.updateClientROPassword(request);
      this.router.navigate(['/']);
    } else {
      const request: IClientResourceOwnerPasswordCreateRequest = {
        validateAudience: this.validateAudience,
        validateCORS: this.validateCORS,
        validateIssuer: this.validateIssuer,
        clientId: this.clientId,
      ...
      this.http.createClientROPassword(request);
    }
  }
}
